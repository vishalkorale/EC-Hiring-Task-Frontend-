...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://intern-task-api.bravo68web.workers.dev/api/register', {
        email,
        password,
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <form onSubmit={handleRegister} className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md sm:max-w-sm">
        <h2 className="text-2xl sm:text-3xl font-semibold mb-6 text-center">Register</h2>
        <input
          type="email"
          value={email}
